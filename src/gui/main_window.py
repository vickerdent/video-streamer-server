...
class MainWindow(QMainWindow):
    ...
    def __init__(self, fallback_mode=None, restore_session=False, last_state=None):
        ...
        self.auto_check_updates = self.settings.value('auto_check_updates', True, type=bool)
    ...
    def on_connection_changed(self, cam_id, connected, info):
        try:
            ...
            if not (1 <= cam_id <= self.running_camera_count): # Use dynamic count
                logger.warning(f"Invalid camera ID: {cam_id} (max: {self.running_camera_count})")
                return
    ...
    def on_frame_received(self, cam_id, frame):
        ...
        if 1 <= cam_id <= max_camera_id and cam_id <= len(self.cameras):
            ...
            logger.debug(f"Frame for camera {cam_id} ignored (have {max_camera_id} widgets)")
    ...
    def update_camera_count(self):
        ...
        if hasattr(self, 'camera_count_label') and hasattr(self, 'camera_count'):
            self.camera_count_label.setText(f"{count}/{self.camera_count} cameras connected")
    ...
    def test_network_callback(self):
        ...
        QTimer.singleShot(3000, lambda: self.on_network_status_changed(True, "192.168.0.198"))
    ...
    def keyPressEvent(self, event):
        ...
        # Ctrl+H to minimize
        if event.matches(QKeySequence.StandardKey.HelpContents):  # This is Ctrl+H on most systems
            pass
