...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Mobile Camera Bridge - OMT/Native Camera Streaming"
    )
    parser.add_argument(
        "--native-camera",
        action="store_true",
        help="Enable native Windows camera device",
    )
    parser.add_argument(
        "--omt", action="store_true", help="Use OMT protocol for vMix (default)"
    )
    parser.add_argument(
        "--bind-ip",
        type=str,
        help="Specific IP address to bind to (e.g., 192.168.1.100)",
    )
    parser.add_argument(
        "--camera-count",
        type=int,
        default=4,
        help="Number of cameras, up to 8 (e.g., 4)",
    )
